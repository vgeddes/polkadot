...
pub mod pallet {
	...
	pub trait Config: configuration::Config + paras::Config + slots::Config {
		...
		#[pallet::constant]
		/// The max number of temporary slots to be scheduled per lease periods to start.
...
mod tests {
	...
	fn run_to_block(n: BlockNumber) {
		...
		AssignedSlots::set_max_temporary_slots(Origin::root(), 6).unwrap();
		AssignedSlots::set_max_permanent_slots(Origin::root(), 2).unwrap();
	}
